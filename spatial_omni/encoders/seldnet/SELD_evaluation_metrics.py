...
class SELDMetrics(object):
    def __init__(self, doa_threshold=20, dist_threshold=np.inf, reldist_threshold=np.inf, nb_classes=11, eval_dist=True,
        ...
        self._DE_TP = np.zeros(self._nb_classes)
        self._DE_TP_dist = np.zeros(self._nb_classes)
        self._DE_FP = np.zeros(self._nb_classes)
        self._DE_FN = np.zeros(self._nb_classes)
    ...
    def compute_seld_scores(self):
        ...
        if self._average == 'micro':
            ...
            DistE = self._total_DistE.sum() / float(self._DE_TP_dist.sum() + eps) if self._DE_TP_dist.sum() else np.nan
            RelDistE = self._total_RelDistE.sum() / float(self._DE_TP_dist.sum() + eps) if self._DE_TP_dist.sum() else np.nan
            LR = self._DE_TP.sum() / (eps + self._DE_TP.sum() + self._DE_FN.sum())
            ...
            DistE = self._total_DistE / (self._DE_TP_dist + eps)
            DistE[self._DE_TP_dist==0] = np.nan
            RelDistE = self._total_RelDistE / (self._DE_TP_dist + eps)
            RelDistE[self._DE_TP_dist==0] = np.nan
            LR = self._DE_TP / (eps + self._DE_TP + self._DE_FN)
        ...
        for frame_cnt in range(len(gt.keys())):
            ...
            for class_cnt in range(self._nb_classes):
                ...
                if class_cnt in gt[frame_cnt] and class_cnt in pred[frame_cnt]:
                    ...
                    if eval_dist:
                        valid_dist_mask = gt_dist[row_inds] >= 0
                        dist_err_list = np.zeros(len(doa_err_list), dtype=np.float64)
                        rel_dist_err_list = np.zeros(len(doa_err_list), dtype=np.float64)
                        dist_thresh_hits = np.zeros(len(doa_err_list), dtype=bool)
                        reldist_thresh_hits = np.zeros(len(doa_err_list), dtype=bool)
                        if np.any(valid_dist_mask):
                            dist_err_list[valid_dist_mask] = np.abs(gt_dist[row_inds][valid_dist_mask] - pred_dist[col_inds][valid_dist_mask])
                            rel_dist_err_list[valid_dist_mask] = dist_err_list[valid_dist_mask] / (gt_dist[row_inds][valid_dist_mask] + eps)
                            dist_thresh_hits[valid_dist_mask] = dist_err_list[valid_dist_mask] > self._dist_T
                            reldist_thresh_hits[valid_dist_mask] = rel_dist_err_list[valid_dist_mask] > self._reldist_T
                    ...
                    TPc = Kc
                    Lc = np.sum(
                        (doa_err_list > self._ang_T)
                        + (dist_thresh_hits if eval_dist else 0)
                        + (reldist_thresh_hits if eval_dist else 0)
                    )
                    FPct = Lc
                    FPc = FPcinf + FPct
                    TPct = Kc - FPct
                    ...
                    self._DE_TP_dist[class_cnt] += np.sum(valid_dist_mask) if eval_dist else 0
