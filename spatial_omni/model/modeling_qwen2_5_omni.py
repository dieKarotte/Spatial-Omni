...
except Exception:  # pragma: no cover - fallback for in-transformers layout
...
class Qwen2_5OmniAudioEncoderOutput(ModelOutput):
    """
    Base class for Qwen2_5Omni audio encoder's outputs, with potential for custom features.
    """
...
class Qwen2_5OmniPreTrainedModelForConditionalGeneration(Qwen2_5OmniPreTrainedModel):
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            # device_map='auto' 多卡场景下 cache_position 可能在不同 device 上，统一移到目标 device
...
class Qwen2_5OmniThinkerCausalLMOutputWithPast(ModelOutput):
    ...
    Args:
        ...
        seld_loss (`torch.FloatTensor`, *optional*):
            Auxiliary SELD loss when `seld_labels` are provided.
        seld_logits (`dict`, *optional*):
            SELD logits from the spatial head (event/activity/doa).
    """
    ...
    seld_loss: Optional[torch.FloatTensor] = None
    seld_logits: Optional[dict] = None
...
QWEN_OMNI_AUDIO_INPUTS = r"""
        Args:
            ...
            spatial_features (`torch.FloatTensor`, *optional*):
                legacy spatial features of shape `(batch_size, C_spat, F_spat, T_spat)` or `(batch_size, T_spat, C_spat, F_spat)`.
                If provided, the legacy spatial spatial encoder will be used to compute spatial embeddings.

            spatial_audio (`torch.FloatTensor`, *optional*):
                Raw FOA waveform of shape `(batch_size, T, 4)`. If `spatial_frontend_trainable=True`, the model will
                compute legacy spatial features internally from this tensor.
                NOTE: By default, the processor returns `spatial_audio` and does not compute `spatial_features`.
        """
...
class Qwen2_5OmniAudioEncoder(Qwen2_5OmniPreTrainedModel):
    ...
    def __init__(self, config: Qwen2_5OmniAudioEncoderConfig):
        ...
        # --- Legacy spatial pipeline removed in Spatial-Omni open-source release ---
        self.spatial_feature_extractor = None
        self.spatial_encoder = None
        self.spatial_projection = None
        self.seld_head = None
        if getattr(config, 'spatial_encoder_type', None) in {'foa_conv3d', 'foa_hybrid_cnn'}:
            raise ValueError(
                f"Legacy spatial_encoder_type {config.spatial_encoder_type!r} removed in "
                "Spatial-Omni open-source. Use the SO-Encoder (BEATs) or SELD path via "
                "Qwen2_5OmniSpatialThinkerForConditionalGeneration instead."
            )

        self.gradient_checkpointing = False
        # Initialize weights and apply final processing
        self.post_init()
        # The global post_init uses a generic N(0, 0.02) init. Re-apply a more
        # conservative init for the random removed transformer adapter to avoid
        # early NaNs in attention/FFN.
        if self.spatial_encoder is not None and getattr(self.spatial_encoder, "transformer", None) is not None:
            self.spatial_encoder.transformer.reset_stable_parameters()
            logger.info("Applied stable initialization to removed transformer adapter.")
    ...
    def forward(
        ...
        output_spatial_features: Optional[bool] = None,
        output_seld: Optional[bool] = None,
    ):

        self._fwd_calls = getattr(self, "_fwd_calls", 0) + 1
        ...
        # --- Legacy spatial fusion path removed; SO-Encoder fuses in spatial subclass ---
        spatial_features_output = None
        seld_logits = None
        ...
        return Qwen2_5OmniAudioEncoderOutput(
            last_hidden_state=token_audio,
            spatial_features=spatial_features_output,
            seld_logits=seld_logits,
            # Pass other outputs like hidden_states and attentions if you compute them
        )
...
class Qwen2_5OmniThinkerTextModel(Qwen2_5OmniPreTrainedModel):
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            # device_map='auto' 多卡场景下 cache_position 可能在不同 device 上，
            # 统一移到 causal_mask 所在的 device 以避免跨设备操作报错。
            cache_position = cache_position.to(device)
...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def forward(
        ...
        spatial_features: Optional[torch.Tensor] = None,
        spatial_audio: Optional[torch.Tensor] = None,
        seld_labels: Optional[Dict[str, torch.Tensor]] = None,
        ...
        if input_ids is not None and input_ids.shape[1] != 1:  # Prefill stage
            if input_features is not None:
                ...
                audio_outputs = self.audio_tower(
                    ...
                    output_seld=seld_labels is not None,

                )
                audio_features = audio_outputs.last_hidden_state
                seld_logits = audio_outputs.seld_logits
        ...
        seld_loss = None
        if seld_labels is not None:
            if seld_logits is None:
                raise ValueError("seld_labels were provided but seld_logits is None. Enable spatial_use_seld_head.")
            accdoa_labels = seld_labels.get("accdoa_labels")
            if accdoa_labels is None:
                raise ValueError("seld_labels must include accdoa_labels for ADPIT loss.")
            accdoa_logits = seld_logits["accdoa_logits_flat"]
            seld_loss_fn = MSELossADPIT()
            seld_loss = seld_loss_fn(accdoa_logits, accdoa_labels)
            seld_weight = getattr(self.config.audio_config, "spatial_seld_loss_weight", 1.0)
            loss = seld_loss * seld_weight if loss is None else loss + seld_weight * seld_loss
...
class Qwen2_5OmniForConditionalGeneration(Qwen2_5OmniPreTrainedModel, GenerationMixin):
    ...
    def forward(self, *args, **kwargs):
        # Trainer may pass bookkeeping kwargs (e.g., num_items_in_batch); ignore them.
    ...
    def set_output_embeddings(self, new_embeddings):
        self.thinker.lm_head = new_embeddings
    ...
    def generate(
        ...
        **kwargs,
        ...
        for key, value in kwargs.items():
            ...
            #     thinker_kwargs[key] = value
            #     # and optionally:
            #     talker_kwargs.pop("spatial_features", None)
        ...
        # Get sequences regardless of return type
        # if isinstance(talker_result, torch.Tensor):
        #     _sequences = talker_result
        # elif hasattr(talker_result, "sequences"):
        #     _sequences = talker_result.sequences  # typical when return_dict_in_generate=True
        # elif isinstance(talker_result, (tuple, list)) and len(talker_result) > 0 and isinstance(talker_result[0], torch.Tensor):
        #     _sequences = talker_result[0]  # very defensive fallback
        # else:
        #     raise TypeError(f"Unexpected talker.generate() return type: {type(talker_result)}")

        # Slice off the prompt + final EOS to get codes
        # talker_generate_codes = _sequences[:, talker_input_ids.shape[1] : -1]
