import os
from ..encoders.seldnet import SELD_evaluation_metrics
from ..encoders.seldnet import cls_feature_class
from ..encoders.seldnet import parameters
import numpy as np
...
class ComputeSELDResults(object):
    def __init__(self, params, ref_files_folder=None, ref_file_names=None):
        ...
        self._ref_labels = {}
        ref_files = self._feat_cls._collect_files(self._desc_dir, '.csv')
        if ref_file_names is not None:
            ref_file_names = set(ref_file_names)
            ref_files = [path for path in ref_files if os.path.basename(path) in ref_file_names]
        tqdm.write('Building SELD reference index from {} ({} files)'.format(self._desc_dir, len(ref_files)))
        for ref_path in tqdm(ref_files, desc='Loading ref labels', leave=False):
            ref_file = os.path.basename(ref_path)
            # Load reference description file
            gt_dict = self._feat_cls.load_output_format_file(ref_path, cm2m=True)  # TODO: Reconsider the cm2m conversion
            gt_dict = self._feat_cls.convert_output_format_polar_to_cartesian(gt_dict)
            nb_ref_frames = self._get_reference_num_frames(ref_file, gt_dict)
    ...
    def _get_reference_num_frames(self, ref_file, gt_dict):
        if self._dataset_stats and 'files' in self._dataset_stats:
            file_key = ref_file.replace('.csv', '.npy')
            file_meta = self._dataset_stats['files'].get(file_key)
            if file_meta and 'label_frames' in file_meta:
                return int(file_meta['label_frames'])
        if gt_dict:
            return int(max(gt_dict.keys())) + 1
        return 0

    def _load_prediction_labels(self, pred_path_and_name):
        pred_files_path, pred_file = pred_path_and_name
    ...
    def get_SELD_Results(self, pred_files_path, is_jackknife=False):
        ...
        pred_jobs = [(pred_files_path, pred_file) for pred_file in pred_files]
        with ThreadPool(processes=self._num_workers) as pool:
            for pred_file, pred_labels in tqdm(
                pool.imap(self._load_prediction_labels, pred_jobs),
                total=len(pred_jobs),
        ...
        if is_jackknife:
            ...
            if self.evaluate_distance:
                return ([ER, conf_interval[0]], [F, conf_interval[1]], [AngE, conf_interval[2]],
                        ...
                        [seld_scr, conf_interval[6]], [classwise_results, np.array(conf_interval)[7:].reshape(7, self._feat_cls.get_nb_classes(), 2)
                                                            if len(classwise_results) else []])
            else:
                return [ER, conf_interval[0]], [F, conf_interval[1]], [AngE, conf_interval[2]], [LR, conf_interval[3]], [
                    seld_scr, conf_interval[4]], [classwise_results, np.array(conf_interval)[5:].reshape(5, self._feat_cls.get_nb_classes(), 2) if len(
    ...
    def get_consolidated_SELD_results(self, pred_files_path, score_type_list=['all', 'room']):
        ...
        # collect predicted files info
        pred_files = sorted(os.listdir(pred_files_path))
