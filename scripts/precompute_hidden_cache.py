...
DEFAULT_CACHE_DIR = (
    "${DCASE_BASELINE_REPO}/"
    "prepared_datasets/starss23_foa_plus_29cls_20s/seld_hidden_cache"
)
SAMPLE_RATE = 16000
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Precompute SELD233 hidden-state caches from QA audio files."
    )
...
def main() -> None:
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="precompute_seld_hidden_cache"):
            ...
            for index, audio_path in enumerate(batch["audio_paths"]):
                hidden_length = int(backbone_output.hidden_lengths[index].item())
                feature_length = int(feature_output.feature_lengths[index].item())
                hidden_states = backbone_output.hidden_states[index, :hidden_length].detach().cpu()
                relpath = build_cache_relpath(audio_path)
                out_path = os.path.join(args.cache_dir, relpath)
                torch.save(
                    {
                        "audio_path": os.path.abspath(audio_path),
                        "seld_hidden_states": hidden_states.to(dtype=cache_dtype).contiguous(),
